...
$C_m$ merge our tactics desiderata ($C_t$) with our adaptivity ($C_c$), 
while $C_f$ take the tech tree constraints into account. This can seem tedious but these variables are all to be seen as different states of $C$, representing our army composition.
    ...
    \item $EU^t$ and $EU^{t+1} \in ([0,1]\dots [0,1])$, the $M$ enemy units types ($EU$) at time $t$ and $t+1$, i.e. $EU^t \in [0,1]^M$. Same as above, but for the enemy and at two times ($t$ and $t+1$).
...
\subsubsection{Evaluation of Clusterings}
