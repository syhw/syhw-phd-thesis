...
A \glos{replay} contains all the actions of each players during a game. We used a dataset of replays to see when the players build which buildings. We attributed a label for each player for each game which notes the players \glos{opening}.
...
\caption{Evolution of $\PP(BT|observations,time,\lambda=1)$ in time (seen/observed buildings on the x-axis). Only $BT$ with a probability $> 0.01$ are shown. The numbers in the legend correspond to build trees identifier numbers.}
\label{fig:ttinf}
\end{figure}
