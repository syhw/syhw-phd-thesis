...
\begin{itemize}
    \item $P(S^t|S^{t-1},O_{1:n}^t)$ should be read as ``the probability distribution on the strategies at time $t$ is determined/influenced by the strategy at time $t-1$ and the observations at time $t$''.
    \item $P(T^t|S^t,T^{t-1},O_{1:n}^t)$ should be read as ``the probability distribution on the tactics at time $t$ is determined by the strategy at time $t$, tactics at time $t-1$ and the observations at time $t$''.
    \item $P(A^t|T^t,A^{t-1},O_{1:n}^t)$ should be read as ``the probability distribution on the actions at time $t$ is determined by tactics at time $t$, the actions at time $t-1$ and the observations at time $t$''.
\end{itemize}
%\end{eqnarray*}
...
% & & & & & & Strat. & Tact. & Act. & & \\
Game & Combinatory & Vertical continuity & Horizontal continuity & Partial information & Randomness \\
\hline
Checkers & $b\approx 4-8; d\approx 70$ & few & some & no & no \\
Chess & $b\approx 35; d\approx 80$ & some & few & no & no \\
Go & $b\approx 30-300; d\approx 150-200$ & some & moderate & no & no \\
%Monopoly 
%Battleship
Limit Poker & $b\approx 3$\footnote{fold,check,raise} $;d/hour \in [20\dots240]$\footnote{number of decisions taken per hour} & moderate & few & much & much \\
Time Racing & $b\approx 50-1,000$\footnote{$\{\ddot{x} \times \theta \times \phi\}$ sampling$\times$50Hz}$;d/min \approx 60+$ & much & much & no & no \\
(TrackMania) & & & & & \\
Team FPS & $b\approx 100-2,000$\footnote{\label{samplingFPS}$\{X \times Y \times Z\}$ sampling$\times$50Hz + firing} $;d/min \approx 100$\footnote{\label{apmFPS}60 ``continuous move actions''+ 40 (mean) fire actions per sec} & some & moderate & some & some \\
(Counter-Strike) & & & & & \\
(Team Fortress 2) & & & & & \\
FFPS duel & $b\approx 200-5,000$\footref{samplingFPS} $;d/min \approx 100$\footref{apmFPS} & some & much & some & negligible \\
(Quake III) & & & & & \\
MMORPG & $b\approx 50-100$\footnote{in RPGs, there are usually more abilities than in FPS games, but the player does not have to aim to hit a target, and thus positioning plays a lesser role than in FPS.} $;d/min \approx 60$\footnote{move and use abilities/cast spells} & moderate & much & few & moderate \\
(WoW, DAoC) & & & & & \\
RTS & $b\approx 200$\footnote{atomic dir/unit $\times$ \# units + constructions + productions}$;d/min=APM\approx 300$\footnote{for pro-gamers, counting group actions as only one action}& moderate & some & much & negligible\\
(StarCraft) & & & & & \\
\hline
\end{tabular}
\label{recapgames}
...
\\
Est-ce que je mets des resultats plus ``bruts'' en annexe?
