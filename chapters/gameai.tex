...
On planning under uncertainty, \citet{LTW} used case-based reasoning (CBR) to perform dynamic plan retrieval extracted from domain knowledge in Wargus (Warcraft II clone). \citet{Ontanon2007} base their real-time case-based planning (CBP) system on a plan dependency graph which is learned from human demonstration in Wargus. In \citep{Mishra2008,Ontanon2010}, they used a knowledge-based approach to perform situation assessment and use the right plan, and revize it (integrating learning, planning, and problem solving in CBR). \citet{Trusty2011} used a genetic-algorithm inspired method to mix and optimize existing expert plan, also in Wargus. \citet{Chung05} adapts Monte-Carlo tree search to planning in RTS games and applied it to a capture-the-flag mod of Open RTS. \citet{Balla} applied UCT (MCTS algorithm) to tactical assault planning in Wargus. Reactive planning and goal-driven autonomy (that is, find the less irrelevant goal to follow in unknown situations) are studied in \citep{Weber2010cr,WeberCIG10}.
\citet{NovaBot2011}
\citet{metalevelbehavioradaptrts}
\citet{Churchill2011}
%\citep{Weber2010qf}

On learning and opponent modeling, \citet{schadd2007opponent} used hierarchical classifiers to learn the opponent's behavior in Total Annihilation (Spring). \citet{HsiehS08} learned players' strategy models with CBR by mining replays of StarCraft. \citet{weberStrat} applied data-mining to StarCraft replays to learn to predict strategies.
\citet{HagelbackCIG10}
\citet{Kim2010}
\citet{Kabanza2010}
\citet{HMMstrat_RTS_AIIDE11}
\citet{SYNNAEVE:OpeningPred}
\citet{SYNNAEVE:StratPred}

On spatial and temporal reasoning, \citet{Forbus2002}
\citet{Southey2007}
\citet{Hagelback2009}
\citet{SmithCIG10}
\citet{ButlerCIG10}
\citet{teamCompositionRTS}
\citet{CadenaG11}
\citet{SORTS}
\citet{SYNNAEVE:Micro}
\citet{weber2011aiide}
