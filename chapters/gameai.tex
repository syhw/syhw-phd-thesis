...
\hline
Checkers & $b\approx 4-8; n\approx 70$ & few & some & no & no \\
Chess & $b\approx 35; n\approx 80$ & some & few & no & no \\
Go & $b\approx 30-300; n\approx 150-200$ & some & moderate & no & no \\
%Monopoly 
%Battleship
Limit Poker & $b\approx 3$\footnote{fold,check,raise} $;n/hour \in [20\dots240]$\footnote{number of decisions taken per hour} & moderate & few & much & much \\
Time Racing & $b\approx 50-1,000$\footnote{$\{\ddot{x} \times \theta [\times \phi]\}$ sampling$\times$50Hz}$;n/min \approx 60+$ & much & much & no & no \\
(TrackMania) & & & & & \\
Team FPS & $b\approx 100-2,000$\footnote{\label{samplingFPS}$\{X \times Y \times Z\}$ sampling$\times$50Hz + firing} $;n/min \approx 100$\footnote{\label{apmFPS}60 ``continuous move actions''+ 40 (mean) fire actions per sec} & some & moderate & some & some \\
(Counter-Strike) & & & & & \\
(Team Fortress 2) & & & & & \\
FFPS duel & $b\approx 200-5,000$\footref{samplingFPS} $;n/min \approx 100$\footref{apmFPS} & some & much & some & negligible \\
(Quake III) & & & & & \\
MMORPG & $b\approx 50-100$\footnote{in RPGs, the player does not have to aim and positioning plays a lesser role than in FPS} $;n/min \approx 60$\footnote{move and use abilities/cast spells} & moderate & much & few & moderate \\
(WoW, DAoC) & & & & & \\
RTS & $b\approx 200$\footnote{atomic dir/unit $\times$ \# units + constructions + productions}$;n/min=APM\approx 300$\footnote{for pro-gamers, counting group actions as only one action}& moderate & some & much & negligible\\
(StarCraft) & & & & & \\
\hline
\end{tabular}
\label{recapgames}
...
Finally, we made a quick survey among gamers and regrouped the 108 answers in table~\ref{surveygamers} page~\pageref{surveygamers}. The goal was a to get a grasp on which skills are correlated to which gameplays. The questions that we asked can be found in Appendix~\ref{apdx:survey}. Answers mostly come from good to highly competitive (on a national level) amateurs.
\\
...
TeamFPS (n: 40) & 1.700 & 1.026 & 1.075 & 1.256 & 0.150 & -0.105 \\
FFPS (n: 22) & 2.000 & 1.095 & 1.095 & 1.190 & 0.000 & 0.150 \\
MMORPG (n: 29) & 1.069 & 1.069 & 1.103 & 1.241 & 0.379 & 0.759 \\
RTS (n: 90) & 1.800 & 1.856 & 1.655 & 1.822 & 0.211 & 0.433 \\
\hline
\end{tabular}
\label{surveygamers}
\end{sidewaystable}
