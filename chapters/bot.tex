...
A derivative (child) of the \texttt{BayesianUnit} class is instantiated for each unit that is controlled. A \texttt{BayesianUnit} is a modal \glos{FSM} as shown in Figure~\ref{fig:unit_HFSM}. It presents a simple interface to move and fight (\texttt{micro()}. Some parameter can be specialized depending on the particularities of unit types:
\begin{itemize}
...
Note that we can also (try to) learn specific parameters of this model during games against a given opponent with \glos{EM} to reconstruct all the partial observations.
